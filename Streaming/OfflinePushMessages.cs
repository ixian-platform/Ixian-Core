...
namespace IXICore.Streaming
{
    class OfflinePushMessages
    {
        ...
        public static bool sendPushMessage(StreamMessage msg, bool push)
        {
            ...
            string url = string.Format("{0}/push.php", pushServiceUrl);
        ...
        public static bool fetchPushMessages(bool force = false, bool fireLocalNotification = false)
        {
            ...
            try
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    string url = string.Format("{0}/fetch.php?tag={1}&nonce={2}&sig={3}", pushServiceUrl, receiver, nonce, Crypto.hashToString(sig));
                    ...
                    foreach (string[] str in jsonResponse)
                    {
                        ...
                        try
                        {
                            ...
                            string id = str[0];
                            url = string.Format("{0}/remove.php?id={1}&nonce={2}&sig={3}", pushServiceUrl, id, nonce, Crypto.hashToString(sig));
        ...
        private static bool registerWithPushNotificationServer()
        {
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    string url = string.Format("{0}/register.php", pushServiceUrl);
