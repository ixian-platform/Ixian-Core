    ...
    public class LocalStorage
    {
        ...
        private List<FriendMessage> readMessagesFile(string path)
        {
            lock (messagesLock)
            {
                ...
                {
                    Logging.error("Cannot open chat file. {0}", e.Message);
                    return messages;
                }
        ...
        {
            return long.Parse(Path.GetFileNameWithoutExtension(filename));
        }
        ...
        private bool writeMessages(Address address, int channel, List<FriendMessage> messages)
        {
            ...
            lock (messagesLock)
            {
                ...
                for (int i = 0; i < local_messages.Count;)
                {
                    ...
                    {
                        Logging.error("Cannot create chat file. {0}", e.Message);
                        return false;
                    }
